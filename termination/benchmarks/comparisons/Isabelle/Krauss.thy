...
fun foo :: "bool \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> nat" where
  "foo True (Suc n) m = foo True n (Suc m)"
| "foo True 0 m = foo False 0 m"
| "foo False n (Suc m) = foo False (Suc n) m"
| "foo False n 0 = n"

fun bar :: "nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> nat" where
  "bar 0 (Suc n) m = bar m m m"
| "bar (Suc v) n m = 0"
| "bar k 0 m = 0"

datatype poly = Pc nat | Pinj nat poly | PX poly nat poly
fun add :: "poly \<Rightarrow> poly \<Rightarrow> poly" where
